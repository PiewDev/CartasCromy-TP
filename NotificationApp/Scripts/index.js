...
$(function () {
    ...
    $("#chat-form").submit(function (event) {
        ...
        var msj = $("#chat-message").val();
        if (msj != '')
        {
            var jugador = $("#chat-message").attr("player");
            $("#chat-message").val('');
            juego.server.enviarMensaje(msj, jugador);
        }
    });

    juego.client.refrescarChat = function (mensaje)
    {
        $("#chat-window").append("<li style='list-style: none; margin-bottom: 10px;'>" + mensaje + "</li>");
        $("#chat-window").scrollTop($("#chat-window").innerHeight() + $("#chat-window > li").length * 50);
    }
    ...
    juego.client.mostrarError = function(error)
    {
        $("#error-zone").show();
    ...
    juego.client.cleanErrors = function ()
    {
        $("#error-zone").hide();
    }
    ...
    juego.client.dibujarTablero = function (jugador1, jugador2, mazo) {
        ...
        if (esJugador1) {
            player1 = jugador1;
            player2 = jugador2;
            $("#chat-message").attr("player", jugador1.Nombre);
        }
        else {
            player2 = jugador1;
            player1 = jugador2;
            $("#chat-message").attr("player", jugador2.Nombre);
        }
